...
PRICE_CHANNEL_NPZ = "price_channel.npz"          # 由 gen_price_channel.py 生成
PRICE_METRICS_JSON = "price_channel_metrics.json"
...
def _locate_attachments():
    for _base in MODEL_DIR.parents:
        for _cand in (_base / "C题" / "附件", _base / "附件"):
            if (_cand / "附件1.xlsx").exists():
                return _cand
    raise FileNotFoundError("未找到 附件1.xlsx，请保持 C题/附件/ 目录完整")

ATTACHMENT_DIR = _locate_attachments()
Q2_MODEL_DIR = MODEL_DIR.parents[2] / "问题2" / "deps"
Q2_M1_DIR = Q2_MODEL_DIR
...
def load_price_channel() -> tuple[np.ndarray, np.ndarray]:
    """读取波动电价通道 ``(price_actual, price_fc)``，均形如 ``(365, 144)``。

    问题4 中电价与负载、光伏同为不可预知量：``price_actual`` 是附件4 的实际
    逐日逐时段电价（**只用于结算**），``price_fc`` 是第 $d$ 天 0:00 可用的因果
    预测电价（**只用于 LP 目标与储能套利**）。两者由 ``gen_price_channel.py``
    在 问题4/重算问题2 的模块环境下算好后缓存，本模块只负责读取与校验。
    """
    path = CACHE_DIR / PRICE_CHANNEL_NPZ
    if not path.exists():
        raise FileNotFoundError(
            f"缺少 {path}，请先运行 python gen_price_channel.py 生成波动电价通道"
        )
    with np.load(path) as npz:
        price_actual = np.asarray(npz["price_actual"], dtype=float)
        price_fc = np.asarray(npz["price_fc"], dtype=float)
    if price_actual.shape[1] != N_SLOT or price_fc.shape != price_actual.shape:
        raise ValueError(
            f"电价通道维度异常：actual={price_actual.shape}, fc={price_fc.shape}"
        )
    if not np.isfinite(price_actual).all() or not np.isfinite(price_fc).all():
        raise ValueError("电价通道存在非数值")
    if (price_actual <= 0).any() or (price_fc <= 0).any():
        raise ValueError("电价通道存在非正值")
    return price_actual, price_fc


def load_price_metrics() -> dict:
    """读取波动电价通道的预测精度与口径说明。"""
    path = CACHE_DIR / PRICE_METRICS_JSON
    if not path.exists():
        return {}
