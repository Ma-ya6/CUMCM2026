...
def simulate_year(
    ...
    *,
    ...
    ``price_day`` / ``price_fc`` 为可选的波动电价通道，均形如 ``(n_days, 144)``：
    ``price_day`` 是实际电价（**只用于结算**：计划购电按 $c_t\\min_kx^{(k)}$、
    调整按 $0.5c_t\\Delta^-$ 与 $1.5c_t\\Delta^+$、紧急购电按 $5c_t$），
    ``price_fc`` 是第 $d$ 天 $k$ 时刻可用的预测电价（**只进入 LP 目标与储能
    套利**）。两者都为 ``None`` 时退化为问题3 的原口径——实际电价与决策电价
    同为附件1 的单条固定日内曲线。
    """
    n_days = len(data.dates) if days is None else min(len(data.dates), days)
    price = data.price
    settle_price = (np.tile(price, (n_days, 1)) if price_day is None
                    else np.asarray(price_day, dtype=float))
    decision_price = (settle_price if price_fc is None
                      else np.asarray(price_fc, dtype=float))
    if settle_price.shape != (n_days, N_SLOT) or decision_price.shape != (n_days, N_SLOT):
        raise ValueError(
            f"电价通道维度应为 {(n_days, N_SLOT)}，"
            f"实际 settle={settle_price.shape}, decision={decision_price.shape}"
        )
    errors = {
    ...
    for day in range(n_days):
        ...
        for si, k in enumerate(stages):
            ...
            ctx = StageContext(
                ...
                price=decision_price[day][start:],
                start_energy=energy,
        ...
        chain = settle_period(commitment_path, day_price, "chain")
        two_way = settle_period(commitment_path, day_price, "two_way")
        emergency_cost = float(EMERGENCY_MULTIPLIER * (day_price @ executed["emergency"]))
