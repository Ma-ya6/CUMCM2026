...
SUMMARY = PROJECT / "结果" / "汇总"          # 汇总类指标同时留一份在结果目录
...
def export_problem(arrays, problem, week):
    ...
    source = PROJECT/"结果"/Path("问题4")/folder if problem.startswith("Q4") else PROJECT/"结果"/folder
...
def main():
    ...
    save(pd.DataFrame(g),SUMMARY/"表G_最终档位指标汇总_正式期334天.csv")
    ...
    causality = PROJECT/"验证"/"报告"/"time_tests.json"
    if causality.exists():
