...
# ---------------------------------------------------------------- 常量与路径
BASE = os.path.dirname(os.path.abspath(__file__))          # …\验证\代码\问题1
PRJ = os.path.dirname(os.path.dirname(os.path.dirname(BASE)))
RESULTS = os.path.join(PRJ, "结果", "问题1")
TABDIR = os.path.join(PRJ, "验证", "数据")
...
def rows_feasibility(f):
    """
    把一个口径的检验结果整理成 7 行。
    每行是 (编号, 检验项含判据, 显示值, 用于比对的显示键, 是否通过, 数字格式)。
    显示值是数字时带 Excel 格式；是区间或合并信息时用字符串。
    比对键按显示精度取整，这样两个口径在末位上的一点点差异不会妨碍合并。
    """
    return [
        ("①", "电量平衡残差最大值（应 < 1e-3 kW）",
         f["电量平衡残差最大值 (kW)"], "%.2e" % f["电量平衡残差最大值 (kW)"],
         f["电量平衡残差最大值 (kW)"] < 1e-3, "0.00E+00"),
        ("②", "储能递推残差最大值（应 < 1e-3 kWh）",
         f["储能递推残差最大值 (kWh)"], "%.2e" % f["储能递推残差最大值 (kWh)"],
         f["储能递推残差最大值 (kWh)"] < 1e-3, "0.00E+00"),
        ("③", "储电量范围（应落在 1200 ~ 10800 kWh）",
         "%.4f ~ %.4f" % (f["储电量最小值 (kWh)"], f["储电量最大值 (kWh)"]), None,
         not f["储电量越界"], None),
        ("④", "充放电功率最大值（应 ≤ 5000 kW）",
         "充电 %.4f；放电 %.4f" % (f["充电功率最大值 (kW)"], f["放电功率最大值 (kW)"]), None,
         not f["充放电功率越限"], None),
        ("⑤", "非负性与充放电互斥（x、w ≥ 0 且不同时充放）",
         "x、w 均非负；max min(u,v) = %.4f kW" % f["同时充放电 min(u,v) 最大值 (kW)"], None,
         (not f["购电功率出现负值"]) and (not f["弃光功率出现负值"])
         and f["同时充放电 min(u,v) 最大值 (kW)"] < 1e-6, None),
        ("⑥", "全天能量守恒（左边 = 右边）",
         "左 = 右 = %.4f kWh（相对误差 %.0e）"
         % (f["全天能量守恒 左边 (kWh)"], f["全天能量守恒 相对误差"]), None,
         f["全天能量守恒 相对误差"] < 1e-9, None),
        ("⑦", "充放电量比 Σv/Σu（应 = η_c·η_d = 0.81）",
         f["充放电量比 Σv/Σu"], "%.6f" % f["充放电量比 Σv/Σu"],
         abs(f["充放电量比 Σv/Σu"] - ETA_ROUND) < 1e-9, "0.000000"),
    ]


def block_feasibility(fa, fb):
    """
    两个口径的检验项逐项比对；完全一致就合并成一列，否则并排两列。

    容差说明：结果表 csv 只保留四位小数，残差由舍入后的数算出来必然有
    1e-4 量级的尾巴，所以判据放宽到 1e-3。求解器内存中的残差是精确的 0，
    solve_q1_ab.py 的 validate() 打的就是那个值。
    """
    ra, rb = rows_feasibility(fa), rows_feasibility(fb)
    same = all((a[3] or a[2]) == (b[3] or b[2]) for a, b in zip(ra, rb))
    data, fmts = [], []
    if same:
        for num, name, val, key, ok, f in ra:
            data.append([num, name, val, "通过" if ok else "未通过"])
            fmts.append([None, None, f or "General", None])
        header = ["编号", "检验项（含判据）", "结果", "结论"]
    else:
        for (num, name, va, _, oka, f), (_, _, vb, _, okb, _) in zip(ra, rb):
            data.append([num, name, va, vb, "通过" if (oka and okb) else "未通过"])
            fmts.append([None, None, f or "General", f or "General", None])
        header = ["编号", "检验项（含判据）", LABEL_A, LABEL_B, "结论"]
    return {"subtitle": None, "header": header, "rows": data, "formats": fmts}


NOTES_FEASIBILITY = [
    "数据来源：results\\约束A（固定）_完整结果.csv、results\\约束B（自由）_完整结果.csv。",
    "本表由 validation\\check_model.py 只读结果 csv 生成，不重新求解，属于对解的外部验证。",
    "约束 A 与约束 B 的检验结果逐项一致（两者只差端点约束，不影响可行性），故合并为一列。",
    "①② 电量平衡与储能递推是模型的硬约束。表中数值由结果表 csv（四位小数）反算，残差判据放宽到 1e-3；",
    "   求解器内存中的残差是精确的 0，见 solve_q1_ab.py 的 validate() 输出。",
    "③④⑤ 分别对应储电量上下限、充放电功率上限、变量非负与充放电互斥。",
    "⑥ 全天能量守恒是把 144 个时段的平衡式求和后的整体校验：左边 Σx·Δt + ΣP^fc·Δt + Σv·Δt，",
    "   右边 ΣL·Δt + Σu·Δt + Σw·Δt。它比逐时段残差更严格，能查出 24:00 收口行算重、算漏这类错误。",
...
def main():
    ...
    for num, name, val, key, ok, f in rows_feasibility(fa):
        s = ("%.6g" % val) if isinstance(val, float) else str(val)
        print("  %s %-40s %-40s %s" % (num, name, s, "通过" if ok else "未通过"))
    ra, rb = rows_feasibility(fa), rows_feasibility(fb)
    if all((a[3] or a[2]) == (b[3] or b[2]) for a, b in zip(ra, rb)):
        print("  约束 A 与约束 B 的检验结果在显示精度上逐项一致，表中合并为一列。")
    else:
        print("  注意：两口径结果存在差异，表中并排两列展示。")
