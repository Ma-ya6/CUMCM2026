...
_V = _P(__file__).resolve()
PRJ = _V.parents[2]
SHARED = PRJ / "代码" / "共享"
sys.path.insert(0, str(SHARED / "出图数据"))
...
def main():
    ...
    original_out = ry.OUT_OVERRIDE
    with TemporaryDirectory(prefix="codex_time_tests_") as tmp:
        ry.OUT_OVERRIDE = Path(tmp)
        for problem in ("Q2", "Q3", "Q4-2", "Q4-3"):
            ry.simulate(a, problem, days=3)
            slots = pd.read_csv(ry.OUT_OVERRIDE / f"{problem}_physical_10min.csv")
            daily = pd.read_csv(ry.OUT_OVERRIDE / f"{problem}_daily.csv")
            with np.load(ry.OUT_OVERRIDE / f"{problem}_plans.npz") as z:
                paths = z["paths"]
    ...
    (PRJ / "验证" / "报告" / "time_tests.json").write_text(json.dumps(report, indent=2, ensure_ascii=False), encoding="utf-8")
