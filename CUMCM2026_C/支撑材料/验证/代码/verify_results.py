...
import numpy as np
import pandas as pd
from run_year import ROOT, out_dir, oc
...
def main():
    ...
    manifest = {}
    for p in sorted(PRJ.rglob("*")):
        if p.is_file() and (p.suffix in (".py", ".csv", ".npz", ".json")) and "__pycache__" not in p.parts and p.name != "annual_audit.json":
            manifest[str(p.relative_to(PRJ))] = hashlib.sha256(p.read_bytes()).hexdigest()
    _rep = PRJ / "验证" / "报告"
    _rep.mkdir(parents=True, exist_ok=True)
    (_rep / "annual_audit.json").write_text(json.dumps(dict(checks=report, sha256=manifest), indent=2, ensure_ascii=False), encoding="utf-8")
