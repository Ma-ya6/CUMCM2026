...
def baseline_checks(a):
    ...
    for p in ("Q2","Q3","Q4-2","Q4-3"):
        f=p.replace("Q","问题")
        d=RES/Path("问题4")/f if p.startswith("Q4") else RES/f
        raw=pd.read_csv(d/f"{p}_physical_10min.csv")
        daily=pd.read_csv(d/f"{p}_daily.csv")
        with np.load(d/f"{p}_plans.npz") as z:
            paths=z["paths"]
...
def cheap_causality_tests(a):
    ...
    with TemporaryDirectory(prefix="codex_logic_diagnostic_") as temp:
        temp=Path(temp)
        orig_out=model.OUT_OVERRIDE
        try:
            model.OUT_OVERRIDE=temp/"baseline";model.OUT_OVERRIDE.mkdir()
