...
_BASE0 = os.path.dirname(os.path.abspath(__file__))
PRJ = os.path.dirname(os.path.dirname(os.path.dirname(_BASE0)))
sys.path.insert(0, os.path.join(PRJ, "代码", "问题1"))
import solve_q1_ab as q1          # noqa: E402

from plot_style import (C_MAIN, C_FOURTH, C_GUIDE, style_frame_grid,
...
FIGDIR = os.path.join(PRJ, "图表", "问题1", "检验图")

DT = q1.DT
T = q1.T

C1 = C_MAIN           # 第一组曲线：#005792 深蓝
C2 = C_FOURTH         # 第二组曲线：#009473 青绿
C_BASE = C_FOURTH      # 基准参考线：#9AA4B2
...
def main():
    ...
    # 精简后的表只留四类信息；功率扫描与其余列移入表注
    header = ["实验组", "参数取值", "购电费 (元)", "相对基准", "Σv/Σu"]
    data, fmts = [], []
    data.append(["基准", "—", base["购电费 (元)"], "—", base["Σv/Σu"]])
    fmts.append([None, None, "0.0000", None, "0.000000"])
    power_line = []
    for group, pname, m in rows:
        if group == "充放电功率上限":
            power_line.append("%s → %+.4f%%"
                              % (pname.replace("P_st = ", ""),
                                 (m["购电费 (元)"] - base["购电费 (元)"])
                                 / base["购电费 (元)"] * 100))
            continue
        rel = (m["购电费 (元)"] - base["购电费 (元)"]) / base["购电费 (元)"]
        data.append([group, pname, m["购电费 (元)"], rel, m["Σv/Σu"]])
        fmts.append([None, None, "0.0000", "+0.0000%;-0.0000%;0.0000%", "0.000000"])
    ...
    notes = [
        ...
        "充放电功率上限扫描（未列表）：%s。" % "，".join(power_line),
    ...
    fig, axes = plt.subplots(1, 2, figsize=(11.0, 4.6))   # 恢复最初的比例（约 2.39:1），两栏并排
    ax = axes[0]
