...
与问题2的唯一差别是电价：由附件1的单条固定日内曲线换成附件4的逐日逐时段
波动电价，且电价与负载、光伏同为**不可预知量**——第 d 天 0:00 只能用 j<d 的
历史实际值预测当天 144 时段电价。预测电价只进入储能套利与计划优化，
购电费用始终按实际电价结算（计划购电 c·x，紧急购电 5c·e）。

风险分位数仍取 0.80：计划购电与紧急购电按同一时刻同一个 c 结算，
报童临界比 4c/(4c+c) 中 c 被约去，故该值与电价的取值、分布、可知性均无关。

1 月为预测预热与储能状态递推期，2 月 1 日至 12 月 31 日为正式费用评价期。
"""
...
# 0.80 来自 5 倍紧急电价下的报童临界比 4c/(4c+c)：计划购电与紧急购电按同一
# 时刻同一个 c 结算，c 在临界比中约去，故该值与电价的取值、分布、是否可预知
# 均无关，是事前给定的理论值，不由任何评价期回测挑选；敏感性网格只作事后对照。
Q = 0.80
...
def plot_price(data, price_forecast: np.ndarray) -> None:
    """电价实际与预测的日内形态对比（正式评价期逐日均值）。"""
    mask = np.asarray(data.dates >= db.FORMAL_START)
    dates = data.dates[mask]
    fig, axes = plt.subplots(2, 1, figsize=(11, 6.5))
    axes[0].plot(dates, data.price[mask].mean(axis=1), lw=1.0, label="实际日均价")
    axes[0].plot(dates, price_forecast[mask].mean(axis=1), lw=1.0, alpha=0.8, label="预测日均价")
    axes[0].set_ylabel("电价 / (元/kWh)")
    axes[0].legend(frameon=False)
    axes[0].grid(alpha=0.25)
    slot_hour = (np.arange(db.N_SLOT) + 0.5) / 6
    axes[1].plot(slot_hour, data.price[mask].mean(axis=0), lw=1.5, label="实际日内均价")
    axes[1].plot(slot_hour, price_forecast[mask].mean(axis=0), lw=1.5, alpha=0.8, label="预测日内均价")
    axes[1].set_xlabel("时刻 / h")
    axes[1].set_ylabel("电价 / (元/kWh)")
    axes[1].legend(frameon=False)
    axes[1].grid(alpha=0.25)
    fig.suptitle("波动电价的实际值与因果预测值")
    fig.tight_layout()
    fig.savefig(FIGURE_DIR / "电价预测.png", dpi=220, bbox_inches="tight")
    plt.close(fig)


def risk_sensitivity(data, forecasts: dict, price_forecast: np.ndarray) -> pd.DataFrame:
...
def main() -> None:
    ...
    # 对照口径：若电价可预知（完美电价信息），同一套因果负载/光伏预测与同一
    # 安全余量下的费用；两者之差即"电价不可预知"的代价。
    perfect_price_daily, _ = db.run_dispatch(
        data, forecasts["load"], forecasts["pv"], margins
    )

    table1, table2, emergency, specified_interval = make_specified_tables(daily, interval)
    sensitivity = risk_sensitivity(data, forecasts, price_fc)
    dispatch_numbers = summary(daily)
    forecast_numbers = forecast_summary(data, forecasts)
    perfect_numbers = summary(perfect_price_daily)
    validation = db.validate_physical_results(interval, db.StorageParameters())
    frozen = {
        "model": {
            "name": "波动电价下的季节水平预测+因果分位数安全余量+确定性LP",
            "risk_quantile": Q,
            "risk_quantile_basis": "5倍紧急电价下的报童临界比 4c/(4c+c)，与c无关",
            "weekly_effect_enabled": False,
            "causal_information_set": "第d天0:00仅使用j<d的历史实际电价/负载/光伏",
            "price_information_set": "当天电价不可预知，须由j<d历史电价因果预测",
            "settlement": "计划购电按c·x、紧急购电按5c·e结算，均用当天实际电价；LP目标用预测电价",
            "initial_state": "E_{1,0}=E_0=6000 kWh；随后逐日跨日连续递推",
            "symbol_convention": [
                {"symbol": "c", "meaning": "外网电价（逐日逐时段波动）"},
        ...
        "price_forecast": price_result["metrics"],
        "dispatch": dispatch_numbers,
        "perfect_price_information": {
            "total_cost_yuan": perfect_numbers["total_cost_yuan"],
            "emergency_kwh": perfect_numbers["emergency_kwh"],
            "days_with_emergency": perfect_numbers["days_with_emergency"],
            "perfect_price_minus_forecast_yuan": float(
                perfect_numbers["total_cost_yuan"] - dispatch_numbers["total_cost_yuan"]
            ),
            "note": "电价可预知的对照口径，不属于可实现的策略；负值表示完美电价信息下费用更低，其绝对值即电价不可预知的代价",
        },
    ...
    for day, weights in enumerate(price_result["weights"]):
        if data.dates[day] >= db.FORMAL_START:
            weight_rows.append({"date": data.dates[day], **weights})
    pd.DataFrame(weight_rows).fillna(0.0).to_csv(
        RESULT_DIR / "电价预测权重.csv", index=False, encoding="utf-8-sig"
    )
    ...
    plot_price(data, price_fc)
    export_result4_2.main()
