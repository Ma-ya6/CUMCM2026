# -*- coding: utf-8 -*-
"""
C 题 问题一 · 口径 A：日循环两端固定为 6000 kWh
...
运行：python solve_A_fixed6000.py
依赖：pandas、numpy、scipy、openpyxl
产出：output/问题一_口径A_固定6000/
原附件只读。
=============================================================================
"""
...
OUTDIR = os.path.join(ROOT, "output", "问题一_口径A_固定6000")

T = 144                     # 一天内的时段总数
DT = 1.0 / 6.0              # 时段时长 Δt，单位 h
E_MIN = 1200.0              # 储电量下限 E_min，kWh
E_MAX = 10800.0             # 储电量上限 E_max，kWh
E0 = 6000.0                 # 初始储电量 E_0，kWh
E_END = 6000.0              # 日末储电量 E_T，kWh（本口径固定为 6000）
...
def main():
    ...
    Q = float((sol["x"] * DT).sum())                 # 全天购电量
    C = float((c * sol["x"] * DT).sum())             # 全天购电费
    Q_u = float((sol["u"] * DT).sum())               # 充电总量
    Q_v = float((sol["v"] * DT).sum())               # 放电总量
    Q_pv = float((P_fc * DT).sum())                  # 光伏发电量
    Q_w = float((sol["w"] * DT).sum())               # 弃光电量
    Q_base = float((np.maximum(net, 0.0) * DT).sum())
