...
产出：tables\\最优性检验.xlsx、figures\\图1_对偶变量与边际电价.png
"""
...
def build_blocks(res_list):
    head1 = ["口径", "原始可行残差", "变量越界", "KKT 违反变量数",
             "原始目标 (元)", "对偶目标 (元)", "对偶间隙 (元)", "相对间隙", "结论"]
    rows1, fmts1 = [], []
    for r in res_list:
        rows1.append([r["tag"], r["pr_res"], "否" if not r["in_bounds"] else "无",
                      r["n_bad"], r["primal"], r["dual"], r["gap"], r["rel_gap"],
                      "通过" if r["pass"] else "未通过"])
        fmts1.append([None, "0.00E+00", None, "0", "0.0000", "0.0000",
                      "0.00E+00", "0.00E+00", None])
    block1 = {"subtitle": None, "header": head1, "rows": rows1, "formats": fmts1}

    head2 = ["口径", "变量块", "变量个数", "贴下界个数", "贴上界个数",
             "违反 rc>0⇒贴下界", "违反 rc<0⇒贴上界", "最大 |约化成本|"]
    rows2, fmts2 = [], []
    for r in res_list:
        for bl in r["blocks"]:
            rows2.append([r["tag"], bl["name"], bl["n"], bl["n_lo"], bl["n_hi"],
                          bl["n_bad_pos"], bl["n_bad_neg"], bl["max_rc"]])
            fmts2.append([None, None, "0", "0", "0", "0", "0", "0.00E+00"])
    block2 = {"subtitle": "分变量块的互补松弛明细", "header": head2,
              "rows": rows2, "formats": fmts2}

    head3 = ["口径", "购电时段的影子价格 = 边际电价的最大偏差 (元/kW)",
             "非购电时段 边际电价 − 影子价格 的最小值 (元/kW)"]
    rows3, fmts3 = [], []
    for r in res_list:
        rows3.append([r["tag"], r["dev_max"], r["slack_min"]])
        fmts3.append([None, "0.00E+00", "0.0000"])
    block3 = {"subtitle": "对偶变量的经济学读法",
              "header": head3, "rows": rows3, "formats": fmts3}
...
def build_notes():
    return [
        ...
        "   说明原始解与对偶解同时达到最优，这就是全局最优的数值证书。",
        "影子价格读法：购电时段（x_t > 0）每一度电都靠外网多买，边际成本就是电价本身，故影子价格 = c_t·Δt；",
        "   非购电时段（光伏富余、储能顶负荷）可以从系统内部腾出一度电，边际成本低于电价，故影子价格更低。",
...
def fig(res_list):
    ...
    for ax, r in zip(axes, res_list):
        ...
        ax.set_ylim(0, max(float(r["y"].max()), float(r["mg"].max())) * 1.45)
        ax.grid(False)
        for s in ("top", "right"):
        ...
        ax.set_title(r["tag"], fontsize=11.5, color="#333333", loc="left")
        ax.set_ylabel("元/kW", fontsize=11.5, color="#333333")
        ax.legend(frameon=False, fontsize=10, loc="upper left", borderaxespad=0.8)
    axes[1].set_xlabel("时刻 / h", fontsize=11.5, color="#333333")
    fig.tight_layout(rect=(0, 0.05, 1, 1))
    fig.text(0.5, 0.015, "图 1  对偶变量与边际电价对照（两条线重合的时段，互补松弛取等号）",
             ha="center", va="bottom", fontsize=12, color="#1A1A1A")
    path = os.path.join(FIGDIR, "图1_对偶变量与边际电价.png")
...
def main():
    ...
    tableio.write_table(os.path.join(TABDIR, "最优性检验.xlsx"),
                        ...
                        build_blocks(res_list), notes=build_notes())
