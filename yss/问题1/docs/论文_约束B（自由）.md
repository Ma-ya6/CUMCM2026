# 问题一：典型日微网购电与储能调度模型（约束 B（自由） · 日循环两端相等、取值自由）
...
**推论**：固定端点版本（约束 A（固定））的任意可行解在本模型中均可行
（取 $E_0=E_T=6000$ 即可），因此本模型的最优费用必然**不高于**约束 A（固定）。
这一关系在灵敏度分析中得到数值验证。
...
| 项目 | 约束 A（固定）（固定 6000） | 约束 B（自由）（取值自由） | 差异 |
|---|---:|---:|---:|
...
从可行性关系上看，这一结果也在预期之内：约束 A（固定） 的可行域是约束 B（自由） 的子集，
...
**与约束 A（固定） 的关系**：两个模型的关系是严格的可行域包含关系，
本模型是约束 A（固定） 的松弛。若论文目标是"给出可直接执行的计划"，
建议以约束 A（固定） 为主；若目标是"考察储能容量的充分利用潜力"，
则以约束 B（自由） 为主，并把约束 A（固定） 作为对照。
