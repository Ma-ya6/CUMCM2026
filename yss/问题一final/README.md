...
├── figures/
│   ├── 约束A（固定）/  图1 负载光伏净负荷、图2 储能SOC与充放电功率、图3 计划购电量
│   └── 约束B（自由）/  同名三张
├── validation/
...
│   └── figures/              图1 对偶变量与边际电价（约束A/B 各一张）、图2 参数敏感性
└── docs/
    ├── 论文_约束A（固定）.md
    ├── 论文_约束B（自由）.md
    ├── 约束AB口径对比.md
...
| 2 出图（约束A） | `python plot_q1_three.py "results\约束A（固定）_完整结果.csv" "figures\约束A（固定）" "（约束A）"` | 3 张图 | 约 1 秒 |
| 3 出图（约束B） | `python plot_q1_three.py "results\约束B（自由）_完整结果.csv" "figures\约束B（自由）" "（约束B）"` | 3 张图 | 约 1 秒 |
| 4 模型检验 | `python validation\check_model.py` | `validation\tables\模型检验表.xlsx` | < 1 秒 |
| 5 最优性检验 | `python validation\check_optimal.py` | `validation\tables\最优性检验.xlsx` + 图 1（约束A/B 各一张） | < 1 秒 |
| 6 敏感性 | `python validation\sensitivity.py` | `validation\tables\敏感性结果.xlsx` + 图 2 | < 1 秒 |

第 2、3 步的第三个参数（"（约束A）"）保留只是为了兼容旧命令行，图内不写图题，
标题由文件名承担。
...
| 图的配色 | `plot_q1_three.py` 顶部的颜色常量；图框与网格改 `plot_style.py` 的 `C_FRAME`、`C_MESH` |
...
全套图的色板抽在上一层目录的 **`plot_style.py`**，问题一的三个画图脚本都从那里取色；
以后问题二、三、四的画图脚本也照这个来，避免各画各的。

| 色值 | 角色 | 用在哪 |
|---|---|---|
| `#005792` | `C_MAIN` 主色 | 净负荷、计划购电量、对偶变量、**储电量 SOC 曲线（alpha 0.8）与峰谷标注小字**、敏感性第一条曲线 |
| `#00A3FF` | `C_SECOND` 次色 | 小区负载（与主色同色相、明度差大） |
| `#DDCC77` | `C_THIRD` 第三档 | 沙黄，问题一暂时没用上，留给后续 |
| `#009473` | `C_FOURTH` 第四档 | 青绿，**光伏**、**充电功率**（图 2 右轴柱）、敏感性第二条曲线 |
| `#9D2B2B` | `C_FIFTH` 第五档 | 深红，问题一未用；后续留给风险语义（紧急购电、违约、CVaR） |
| `#4A9FD4` | `C_BLUE_MID` 中蓝 | 备用色，问题一未用 |
| `#E57373` | `C_RED_LIGHT` 浅红 | **放电功率**（图 2 右轴柱，透明度 0.42） |
| `#8ECAE6` | `C_BLUE_LIGHT` 淡蓝 | 备用色，问题一未用 |
| `#B39DDB` | `C_PURPLE_LIGHT` 浅紫 | 备用色，问题一未用 |
| `#6e2277` | `C_PURPLE` 紫色 | **边际电价**（检验图 1） |
| `#9AA4B2` | `C_GUIDE` 参考线 | 零线、SOC 上下限、基准线（中性灰，唯一不在五色内） |

充电柱用 `C_FOURTH` 加 `alpha=0.30`，放电柱用 `C_RED_LIGHT` 加 `alpha=0.42`；
储电量 SOC 曲线用 `C_MAIN` 加 `alpha=0.8`，同色标注小字用满不透明的 `C_MAIN`。

配套的样式约定也写在 `plot_style.py` 的文档串里：图例放图内且不带边框、
**四周细黑框（`C_FRAME`）+ 内部细灰网格（`C_MESH`，横竖都画，压在所有数据下面）**、
数据线加粗（主 2.8 / 次 2.2）、图内不写图题（标题由文件名承担）。
画框和网格统一走 `plot_style.style_frame_grid()`，问题一的正文图与检验图都调这一个函数；
双轴图（图 2）的网格挂在下层的右轴上，柱子是底纹上的色块，不会被网格线划开。

引用方式：

```python
import os, sys
sys.path.insert(0, r"C:\Users\21187\Desktop\数模")
from plot_style import C_MAIN, C_SECOND, C_FOURTH, C_GUIDE, C_TEXT
from plot_style import apply_base_style, style_frame_grid

apply_base_style()
style_frame_grid(ax)                    # 四周细黑框 + 内部细灰网格
ax.plot(x, y, color=C_MAIN, lw=2.8)
```
