...
def stack(name):
    ...
    sys.path[:] = [p for p in sys.path if not str(p).startswith(str(ROOT / "deps"))]
    sys.path.insert(0, str(ROOT / "deps" / name))
    return importlib.import_module("deterministic_baseline")

def fingerprint():
    h = hashlib.sha256()
    for p in sorted((ROOT / "deps").rglob("*.py")):
        h.update(p.read_bytes())
    h.update(Path(__file__).read_bytes())
    h.update((ROOT / "opt" / "opt_core.py").read_bytes())
    for p in sorted((ROOT.parent / "附件").glob("*.xlsx")):
        h.update(p.read_bytes())
    return h.hexdigest()
...
def prepare():
    ...
    stack("Q2")
    sys.path.insert(0, str(ROOT / "deps" / "Q3"))
    import forecasts
...
def official_points(a):
    ...
    raw = pd.read_excel(ROOT.parent / "附件" / "附件3.xlsx")
...
def simulate(a, problem, days=None):
    ...
    result = dict(problem=problem, simulation_days=n, formal_days=len(formal),
                  ...
                  source="original attachments rebuilt in 234codex版; no inherited result cache")
...
def main():
    ...
    if not args.days:
        saved = [json.loads(p.read_text(encoding="utf-8")) for p in sorted(OUT.glob("*_summary.json"))]
        saved = [s for s in saved if s["simulation_days"] == 365]
        (OUT / "frozen_numbers.json").write_text(json.dumps(saved, indent=2, ensure_ascii=False), encoding="utf-8")
        ...
        (ROOT / "全年费用汇总.md").write_text(text, encoding="utf-8")
