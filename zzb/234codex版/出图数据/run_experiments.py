...
from export_plot_data import ROOT,TABLES,TAGS,FORMAL,load_arrays,save,cvar95,forecasts_and_reserves
from experiment_backend import simulate

def signature(a, problem, **settings):
    h=hashlib.sha256()
    for p in (HERE/"experiment_backend.py",HERE/"run_experiments.py",ROOT/"run_year.py",ROOT/"opt"/"opt_core.py",ROOT/"cache"/"rebuilt.npz"):
        h.update(p.read_bytes())
    h.update(json.dumps(dict(problem=problem,**settings),sort_keys=True).encode())
    return h.hexdigest()

def baseline(problem):
    daily=pd.read_csv(ROOT/"results"/f"{problem}_daily.csv")
    slots=pd.read_csv(ROOT/"results"/f"{problem}_physical_10min.csv")
    assert len(daily)==365 and len(slots)==52560
    return daily,slots

def run_cached(a,problem,label,*,q=.8,oracle_load_pv=False,oracle_price=False):
    folder=HERE/"补跑输出"/label/problem
