"""七个互不相同的**决策模型**，统一接口 ``plan(StageContext) -> 购电向量``。

预测层、物理执行、经济结算三者全部固定（见 ``dispatch_core``），差别只在
"给定同一份预报，怎么定承诺购电量"。七个模型分属不同建模范式：

| 代号 | 模型 | 范式 | 风险处理 |
| --- | --- | --- | --- |
| M1 | 确定性经济调度 LP | 确定性优化 | 无（预报即真值） |
| M2 | 分位安全余量 LP | 确定性优化 | 约束侧加历史分位余量（模型3 口径） |
| M3 | 两阶段随机规划 | 随机优化 | 场景期望，紧急购电为追索 |
| M4 | 均值–CVaR 随机规划 | 风险厌恶随机优化 | 期望 + 尾部风险项 |
| M5 | 鲁棒优化 | 鲁棒优化 | 盒式不确定集取历史极端残差 |
| M6 | 精确链式结算 MILP | 混合整数优化 | 分位余量 + 结算项精确（不用凸化） |
| M7 | 规则型启发式 | 无优化 | 凭规则留量，不显式建模不确定性 |

## 为什么 M2 与 M6 单独列出

模型3 的 LP 把链式结算的 $c\\min\\{x_{\\text{prev}},x\\}$ 锚定凸化成
$0.5cx+1.0ce$。M2 沿用该凸化，M6 用 0-1 变量精确刻画 $\\min$，两者共用同一份
分位余量，故 M2 与 M6 的差额**就是凸化近似的代价**，其余模型之间的差额则是
建模范式的差别。

## 共同的经济口径

购电承诺 $x$ 的成本按下单时刻的**边际**计：相对上一版承诺，下调部分半价
（$0.5c$）、上调部分 1.5 倍（$1.5c$）。用不完的计划电量不额外计费（链式基准
$c\\min_k x^{(k)}$ 已经付过），缺口以 $5c$ 紧急购电。四个模型（M1–M5、M6）
...
def solve_scenario_lp(
    ...
    *,
    ...
    if ref is None:
        ...
        # 锚定边际：0.5c·x + 1.0c·eb。系数落在 i_eb 还是 i_eo 并非笔误：
        # 模型3 的 solve_stage 里这两个块的命名与注释是反的，实际上
        # ``i_eb`` 对应上调量（x−ref）、``i_eo`` 对应下调量（ref−x），
        # 1.0c 加在前者上。要复现冻结值就必须照抄这份矩阵。
    ...
    if ref is not None:
        for t in range(n):
            ...
            # 目标系数落在 i_eb（而非 i_eo）不是笔误：模型3 用不等式
            # eb ≥ x−ref、eo ≥ ref−x，罚的是 eo=(ref−x)⁺，这里必须一一对应。
            row = np.zeros(nvar)
...
class QuantileLP(DecisionModel):
    ...
    name = "分位安全余量 LP"
    paradigm = "确定性优化"
    risk = "约束侧加 q=0.80 历史分位余量（模型3 口径）"
...
class RobustLP(DecisionModel):
    ...
    name = "鲁棒优化"
    paradigm = "鲁棒优化"
    risk = "盒式不确定集取历史累计残差最大的整条日曲线，对最坏情形求可行解"
