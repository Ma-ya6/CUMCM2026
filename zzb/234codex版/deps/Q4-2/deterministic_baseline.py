...
class DataBundle:
    ...
    load_kw: np.ndarray
    pv_kw: np.ndarray
    # 问题4：电价为附件4的逐日逐时段实际电价 (365, N_SLOT)，而非单条固定曲线。
    price: np.ndarray
    initial_load: np.ndarray
    initial_pv: np.ndarray
    # 附件1的单条电价曲线，与 initial_load/initial_pv 同构，只作第 0 天冷启动先验。
...
def load_inputs() -> DataBundle:
    ...
    price_raw = pd.read_excel(attachment4)
    ...
    initial_price = pd.to_numeric(base_raw.iloc[:, 1], errors="raise").to_numpy(float)
    ...
    if not price_dates.equals(dates):
        raise ValueError("附件4与附件2的日期序列不一致")
    if len(initial_price) != N_SLOT:
        raise ValueError(f"附件1电价长度异常：{len(initial_price)}")
    ...
    return DataBundle(
        dates, load_kw, pv_kw, price, initial_load, initial_pv, initial_price
    )
...
def solve_full_year_oracle(
    ...
    price_all = data.price.reshape(-1)
    step_max = params.p_max_kw * DT
    n = len(load)
    q0, c0, d0, w0, s0 = 0, n, 2 * n, 3 * n, 4 * n
    nvar = 5 * n
...
def run_dispatch(
    ...
    price_forecast: np.ndarray | None = None,
) -> tuple[pd.DataFrame, pd.DataFrame]:
    """逐日滚动调度。

    问题4中电价与负载同为不可预知量：``price_forecast`` 是第 d 天 0:00 可用的
    电价预测，只用于 LP 目标与储能套利；购电费用始终按 ``data.price`` 的实际
    电价结算。传 None 表示直接使用实际电价，仅用于完美电价信息的对照口径。
    """
    ...
    for day, date in enumerate(data.dates):
        ...
        plan_cost = float(data.price[day] @ plan["purchase"])
        emergency_cost = float(5.0 * data.price[day] @ actual["emergency"])
