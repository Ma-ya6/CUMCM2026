...
def main():
    ...
    for path in sorted((HERE/'补跑输出').rglob('*_summary.json')):
    ...
    lp_evidence=[]
    for path in sorted((HERE/'补跑输出'/'C0').glob('*.json')):
    ...
    evidence['numerical_adapters']=[dict(file=str(path.relative_to(ROOT)),
        ...
        for path in sorted((HERE/'补跑输出').rglob('numerical_adapter.json'))]
    ...
    for path in sorted(TABLES.rglob('*.csv')):
        df=pd.read_csv(path)
        files.append(dict(file=str(path.relative_to(HERE)),rows=len(df),columns=len(df.columns),
    ...
    (ROOT/'audits'/'DF完成核验.json').write_text(json.dumps(evidence,ensure_ascii=False,indent=2),encoding='utf-8')
    ...
    (ROOT/'audits'/'DF补算汇总.md').write_text('\n'.join(lines),encoding='utf-8')
