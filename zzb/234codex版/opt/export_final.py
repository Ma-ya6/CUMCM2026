...
# 输入附件的时间点是右端点；例如10:00-10:10对应输入时间点10:10，
# 即零基第60列。结果附件的区间表头只用于展示，不能按同名时间点取值。
SIX_SLOTS = [60, 72, 84, 96, 108, 120]
...
def _clock_label(total_minutes: int) -> str:
    """Return a zero-padded clock label, explicitly marking the next day."""
    day, minute = divmod(total_minutes, 24 * 60)
    label = f"{minute // 60:02d}:{minute % 60:02d}"
    return label if day == 0 else f"{label}+{day}"


def _physical_period(t: int) -> str:
    """Map zero-based input column ``t`` to its physical ten-minute interval."""
    start = t * 10
    return f"{_clock_label(start)}-{_clock_label(start + 10)}"
...
def main() -> None:
    ...
    try:
        if problem in ("Q2", "Q4-2"):
            ...
            for day in range(n_days):
                for k in rt.STAGES:
                    s = rt.stage_start(k)
                    load_fc[day, s:] = rt.load_at_stage(data, bundle, day, k, "own")[s:]
                    pv_fc[day, s:] = rt.pv_at_stage(data, bundle, day, k, "fused")[s:]
    ...
    _assert_current_cost(problem, tag, float(daily.total_cost_yuan.sum()))
    ...
    for j, i in enumerate(keep):
        ...
        for t in range(oc.N_SLOT):
            rows.append({
                ...
                "source_time": _clock_label((t + 1) * 10),
                "time_period": _physical_period(t),
...
def _assert_current_cost(problem: str, tag: str, cost: float) -> None:
    """Compare export rerun with the latest time-consistent daily result, if present."""
    path = OPT_DIR / "out" / f"{problem}_{tag}_逐日.csv"
    if not path.exists():
        return
    expected = float(pd.read_csv(path)["total_cost_yuan"].sum())
    if abs(cost - expected) >= 1.0:
        raise AssertionError(
            f"{problem}/{tag} 导出重跑总费用 {cost:.2f} 与当前逐日结果 {expected:.2f} 不一致")
