...
必须复现冻结值 13,748,164.50 元；否则说明底座被改动过，结果不可比。
"""
...
MODEL3_FROZEN = Path(__file__).resolve().parent.parent / "模型3" / "results" / "frozen_numbers.json"
...
def archive(table: pd.DataFrame) -> None:
    ...
    new = table.drop(columns=["相对M2_元", "相对M2_%"], errors="ignore").copy()
...
def main() -> None:
    ...
    frozen = json.loads(MODEL3_FROZEN.read_text(encoding="utf-8"))
    target = float(frozen["final_total_cost_yuan"])
