...
def errors_for(data, bundle, dispatch_core, *, perfect: bool = False) -> dict:
    ...
    """
    return {
        k: dispatch_core._error_matrix(
            data, bundle, k, "own", "perfect" if perfect else "fused")
        for k in STAGES
    }
...
def simulate(
    ...
    for day in range(n_days):
        ...
        for si, k in enumerate(stages):
            start = k * oc.SLOTS_PER_HOUR
            is_last = si == len(stages) - 1
            load_full = dispatch_core._load_at_stage(data, bundle, day, k, load_src)
            pv_full = dispatch_core._pv_at_stage(data, bundle, day, k, pv_src)
            ...
            exec_end = oc.N_SLOT if is_last else stages[si + 1] * oc.SLOTS_PER_HOUR
            if exec_end > start:
